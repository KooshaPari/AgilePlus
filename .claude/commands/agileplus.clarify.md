...
1. Run `agileplus agent feature check-prerequisites --json --paths-only` from the repository root and parse JSON for:
   ...
   - If command fails or JSON parsing fails, abort and instruct user to run `/agileplus.specify` first or verify they are in an agileplus-initialized repository.
