...
| Credentials | `crates/agileplus-domain/src/credentials/{file,keychain,factory,store}.rs` contains multiple persistence paths. | Production must forbid file-backed plaintext secrets and prove keychain retrieval/redaction. |
...
1. Local development may use explicit non-production development credentials, but no
   production profile may write secret bytes to files, logs, database rows, events, or
   test fixtures.
