...
After systematic evaluation informed by SOTA research (see SOTA_RESEARCH_POLYINFRA.md), we have selected the following technology stack:

### Core Implementation

**Primary Language**: Go (NVMS orchestrator), Rust (performance-critical components)
- Go: Concurrency model suited for orchestration, rich cloud SDKs
- Rust: Memory safety without GC overhead, Firecracker integration
- Both: Strong type systems, excellent performance, rich ecosystems

### Container Orchestration & Virtualization

| Component | Selection | Benchmarks |
|-----------|-----------|------------|
| **Primary** | NVMS (Firecracker microVMs) | 125ms startup, 5-15MB memory |
| **macOS Fallback** | OrbStack | 2x CPU, 6x lower memory than Docker Desktop |
| **Linux/Windows** | Podman | Daemonless, rootless, 10-15% lower memory |
| **Compat Layer** | Docker Compose | Generated from NVMS for legacy support |

**Rationale**: 
- NVMS provides 3-tier isolation (WASM/gVisor/Firecracker) unmatched by containers
- OrbStack fills macOS gap when NVMS unavailable (<1s startup vs 10-30s Docker)
- Podman eliminates daemon requirement, aligns with NVMS philosophy

### Storage & Database Backends

| Use Case | Selection | Alternative | Why Selected |
|----------|-----------|-------------|--------------|
| **Cache/Sessions** | Dragonfly | Redis | 25x faster (1.2M vs 150K ops/sec), 30-50% less memory |
| **Time-Series** | QuestDB | InfluxDB | 100x faster queries, SQL native, Apache license |
| **Vector Search** | Qdrant | Pinecone | Self-hosted, 15ms p99 latency, Apache license |
| **Messaging** | NATS | Redis Pub/Sub | 10M msg/sec, persistence via JetStream |
| **Object Storage** | MinIO | AWS S3 | 10x faster, self-hosted, S3-compatible |

**Rationale**: 
- Dragonfly: BSL license acceptable for 25x performance gain; fallback to KeyDB/Valkey available
- QuestDB: SQL interface critical for observability queries; TimescaleDB migration path if needed
- Qdrant: $70/mo+ Pinecone cost unacceptable; pgvector too slow (45ms vs 15ms)
- NATS: Redis Pub/Sub not a real message queue (no persistence)
- MinIO: AGPL acceptable for infrastructure; eliminates S3 latency (100-500ms → 10-20ms)

### Key Libraries and Frameworks

1. **Configuration Management**
   - NVMS YAML format with schema validation
   - Environment-based overlay via `${env:VAR}` syntax
   - AWS resource references via `${aws:SERVICE:RESOURCE}`

2. **Observability**
   - Structured logging with trace correlation (tracely)
   - Metrics collection via QuestDB time-series
   - Distributed tracing with W3C Trace Context

3. **Data Handling**
   - Message serialization: Protocol Buffers (gRPC) + JSON (REST)
   - Validation: JSON Schema for NVMS, Pydantic for Python SDKs
   - Caching: Dragonfly with automatic TTL management

4. **Testing**
   - Unit testing: Native frameworks (Go testing, Rust test)
   - Integration: Testcontainers with Podman backend
   - Load testing: k6 with QuestDB metrics ingestion

### Infrastructure and Operations

1. **Containerization**: Multi-runtime via PhenoCompose (NVMS primary, OrbStack/Podman fallback)
2. **Orchestration**: NVMS 3-tier isolation with automatic tier selection
3. **CI/CD**: GitHub Actions with container matrix (Docker, Podman, OrbStack)
4. **Monitoring**: QuestDB for metrics, Qdrant for log indexing, Grafana dashboards
...
3. **Vendor Dependencies**: Some components tied to specific vendors/platforms (mitigated by SOTA research - see SOTA_RESEARCH_POLYINFRA.md for benchmarks and alternatives)
