...
use agileplus_domain::domain::module::Module;
    ...
    fn seed_module(conn: &Connection, id: i64) {
        let slug = format!("mod-{id}");
        conn.execute(
            "INSERT OR IGNORE INTO modules (id, slug, friendly_name, created_at, updated_at) \
             VALUES (?1, ?2, ?3, datetime('now'), datetime('now'))",
            params![id, slug, format!("Module {id}")],
        )
        .unwrap();
    }
    ...
    fn tag_is_idempotent() {
        ...
        let tag = ModuleFeatureTag::new(20, 10);
        tag_feature_to_module(&conn, &tag).unwrap();
        tag_feature_to_module(&conn, &tag).unwrap(); // should not error
    }
