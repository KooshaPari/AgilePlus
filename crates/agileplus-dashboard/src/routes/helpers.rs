...
mod tests {
    ...
    use super::{DashboardFilter, build_kanban_cards, feature_matches_filter};
    use crate::app_state::DashboardStore;

    #[test]
    fn feature_filters_distinguish_active_and_shipped_lifecycle_states() {
        let store = DashboardStore::seeded();
        let shipped = store
            .features
            .iter()
            .find(|feature| feature.id == 1)
            .expect("seeded shipped feature");
        let active = store
            .features
            .iter()
    ...
    #[test]
    fn blocked_kanban_filter_uses_linked_work_packages_and_active_project_scope() {
        let mut store = DashboardStore::seeded();
        store
            .work_packages
            .get_mut(&4)
            .expect("seeded feature 4 work packages")[0]
            .state = WpState::Blocked;

        let blocked_cards = build_kanban_cards(&store, DashboardFilter::Blocked);
        let visible_ids: Vec<_> = blocked_cards
            .values()
            .flatten()
        ...
        store
            ...
            .project_id = Some(2);
        let scoped_cards = build_kanban_cards(&store, DashboardFilter::Blocked);
        assert!(scoped_cards.values().all(Vec::is_empty));
    }
}
