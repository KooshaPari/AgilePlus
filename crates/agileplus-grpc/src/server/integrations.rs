...
use agileplus_domain::ports::{
    AgentPort, ContentStoragePort, ObservabilityPort, ReviewPort, StoragePort, VcsPort,
};
    ...
    O: ObservabilityPort + 'static,
{
    ...
    async fn list_backlog_items_impl(
        ...
        let filters = BacklogFilters {
            intent: (!request.type_filter.is_empty())
                .then(|| request.type_filter.parse())
                .transpose()
                .map_err(Status::invalid_argument)?,
            status: (!request.state_filter.is_empty())
                .then(|| request.state_filter.parse())
                .transpose()
                .map_err(Status::invalid_argument)?,
    ...
    O: ObservabilityPort + 'static,
{
    async fn sync_feature_to_plane(
        &self,
        _: tonic::Request<SyncFeatureToPlaneRequest>,
    ) -> Result<Response<SyncFeatureToPlaneResponse>, Status> {
        Err(Status::unimplemented(
            "Plane sync is not implemented by the core",
        ))
    }
    async fn sync_wp_to_plane(
        &self,
        _: tonic::Request<SyncWpToPlaneRequest>,
    ...
    async fn detect_plane_conflicts(
        &self,
        _: tonic::Request<DetectPlaneConflictsRequest>,
    ...
    async fn sync_bug_to_git_hub(
        &self,
        _: tonic::Request<SyncBugToGitHubRequest>,
    ) -> Result<Response<SyncBugToGitHubResponse>, Status> {
        Err(Status::unimplemented(
            "GitHub sync is not implemented by the core",
        ))
    }
    async fn sync_issue_status(
        &self,
        _: tonic::Request<SyncIssueStatusRequest>,
    ...
    async fn detect_git_hub_conflicts(
        &self,
        _: tonic::Request<DetectGitHubConflictsRequest>,
    ...
    async fn classify_input(
        &self,
        _: tonic::Request<ClassifyInputRequest>,
    ) -> Result<Response<ClassifyInputResponse>, Status> {
        Err(Status::unimplemented(
            "input classification is not implemented by the core",
        ))
    }
    async fn create_backlog_item(
        &self,
        request: tonic::Request<CreateBacklogItemRequest>,
    ) -> Result<Response<CreateBacklogItemResponse>, Status> {
        self.create_backlog_item_impl(request.into_inner()).await
    }
    async fn list_backlog(
        &self,
        request: tonic::Request<ListBacklogRequest>,
    ) -> Result<Response<ListBacklogResponse>, Status> {
        self.list_backlog_items_impl(request.into_inner()).await
    }
    async fn promote_backlog_item(
        &self,
        _: tonic::Request<PromoteBacklogItemRequest>,
    ) -> Result<Response<PromoteBacklogItemResponse>, Status> {
        Err(Status::unimplemented(
            "backlog promotion is not implemented by the core",
        ))
    }
    async fn generate_router(
        &self,
        _: tonic::Request<GenerateRouterRequest>,
    ) -> Result<Response<GenerateRouterResponse>, Status> {
        Err(Status::unimplemented(
            "router generation is not implemented by the core",
        ))
    }
}
