...
4. Artifact operations read/write files in `agileplus-specs/<feature>/` paths.
...
- **Artifact layout**: All spec artifacts live in `agileplus-specs/<feature>/` as defined in `data-model.md` "Git Artifact Layout".
...
**Purpose**: Read and write spec/plan/audit artifacts in the `agileplus-specs/<feature>/` directory structure.
...
2. **read_artifact**:
   ...
   ```
   - Compute full path: `<repo_root>/agileplus-specs/<feature_slug>/<relative_path>`
...
5. **scan_feature_artifacts**:
   ...
   ```
   - Base path: `<repo_root>/agileplus-specs/<feature_slug>/`
...
2. Implement a scanning function:
   ...
   ```
   - List directories under `agileplus-specs/` in the working tree.
...
3. Implement commit history extraction:
   ...
   ```
   - Walk the commit log filtered to paths under `agileplus-specs/<feature_slug>/`.
