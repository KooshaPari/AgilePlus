...
Sub-commands are seeded from a hybridized superset of reference framework commands (agileplus, bmad, gsd, openspec), then pruned and adapted to AgilePlus's architecture. Agents invoke them via Claude Code's SlashCommand tool. Each invocation is audit-logged.
