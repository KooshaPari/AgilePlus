...
# /agileplus.implement - Create Workspace for Work Package
...
```bash
agileplus agent workflow implement WP## --agent __AGENT__
```
...
✓ Implementation complete and tested:
  agileplus agent tasks move-task WP## --to for_review --note "Ready for review"
```

**Step 2**: Create the workspace (if needed) and implement according to the prompt
```bash
agileplus implement WP##              # No dependencies (branches from main)
agileplus implement WP## --base WPXX  # With dependencies (branches from base WP)
```
...
4. ✅ **WP moved to for_review lane**: `agileplus agent tasks move-task WP## --to for_review --note "Ready for review"`
...
After `/agileplus.tasks` generates work packages in the main repository:
- Planning artifacts (spec, plan, tasks) are already in main
- Run `agileplus agent workflow implement WP01 --agent __AGENT__` to get the full prompt
- Run `agileplus implement WP01` to create a workspace for the first WP
- Run `agileplus implement WP02 --base WP01` if WP02 depends on WP01
...
```
/agileplus.specify → Creates spec.md in main
/agileplus.plan → Creates plan.md in main
/agileplus.tasks → Creates tasks/*.md in main
```

**Implementation Phase** (creates worktrees on-demand):
```
agileplus implement WP01 → Creates .worktrees/###-feature-WP01/
agileplus implement WP02 --base WP01 → Creates .worktrees/###-feature-WP02/
```
...
#   agileplus agent tasks move-task WP## --to for_review --note "..."

# 3. Create workspace (if not exists)
agileplus implement WP##              # Or with --base if dependencies
...
- `planned` → Initial state after `/agileplus.tasks`
...
**Check current lane**:
```bash
grep "^lane:" agileplus-specs/###-feature/tasks/WP##-*.md
```

## Troubleshooting

**Error: "Base workspace WP01 does not exist"**
- Solution: Implement WP01 first: `agileplus implement WP01`

**Error: "WP02 has dependencies. Use: agileplus implement WP02 --base WP01"**
...
- Check: Did you move to for_review? `agileplus agent tasks move-task WP## --to for_review`
